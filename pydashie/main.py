...
def application_css():
    ...
    for path in paths:
      for name in glob.glob(path):
        scripts.append(name)
    log.info('Found the following CSS/SCSS files: \n %s' % '\n'.join(scripts))

    output = []
    for script in scripts:
        output.append('/* Begin SCSS: %s */\n' % script)
        if '.scss' in script:
          log.info('Compiling scss for %s' % script)
          contents = sass.compile(filename=script)
        else:
          f = open(script)
          contents = f.read()
          f.close()
...
def send_static_fonts(filename):
